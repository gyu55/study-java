...
//		column명이 많으면 헷갈릴 수 있으니 key name으로도 가능
...
//   회원탈퇴
   public void deleteAccount() {
	   ...
	   try {
		   ...
		   preparedStatement.setLong(1, session);
		   preparedStatement.executeUpdate();
		   System.out.println("회원탈퇴 끝났다. 잘 가라.");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}finally {
...
//   
   public ArrayList<MemberVO> getRecommenders() {
	    String query = "SELECT ID, MEMBER_EMAIL, MEMBER_ADDRESS "
	            + "FROM TBL_MEMBER "
	            + "WHERE MEMBER_RECOMMENDER_EMAIL = "
	            + "(SELECT MEMBER_EMAIL FROM TBL_MEMBER WHERE ID = ?)";
	    ...
	    try {
	        ...
	        System.out.println("나를 추천한 사람 수: " + memberList.size());
...
// 내가 추천한 추천인 조회
   public void referee() {
	   String query = "SELECT * FROM TBL_MEMBER WHERE MEMBER_RECOMMENDER_EMAIL = ?";
	   connection = DBConnecter.getConnect();
	   try {
	        preparedStatement = connection.prepareStatement(query);
	        preparedStatement.setLong(1, session);
